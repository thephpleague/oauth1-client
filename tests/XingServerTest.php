...
class XingTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testGettingTemporaryCredentials()
    {
        ...
        $client->shouldReceive('sendRequest')->with(m::on(function(RequestInterface $request) use ($me) {
            ...
            $me->assertSame( 'https://api.xing.com/v1/request_token', $url);
    ...
    public function testGettingTokenCredentials()
    {
        ...
        $client->shouldReceive('sendRequest')->with(m::on(function(RequestInterface $request) use ($me) {
            ...
            $me->assertSame('https://api.xing.com/v1/access_token', $url);
    ...
    public function testGettingUserDetails()
    {
        ...
        $client->shouldReceive('sendRequest')->with(m::on(function(RequestInterface $request) use ($me) {
            $headers = $request->getHeaders();
            $url = $request->getUri()->__toString();
            $me->assertSame('https://api.xing.com/v1/users/me', $url);
